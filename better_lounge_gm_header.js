...
function main()
{
  GM.getValue("betterLoungeConfiguration", "{}").then(afterGetValue);
}

function afterGetValue(configurationJSON)
{
  console.log("Loaded stored configuration: " + configurationJSON);
  var configuration = JSON.parse(configurationJSON);
  var autoCollapseLevel;
  var refreshAfterSave;
  if (typeof configuration.autoCollapseLevel === 'undefined') {
    autoCollapseLevel = defaultConfiguration.autoCollapseLevel;
  }
  else {
    autoCollapseLevel = configuration.autoCollapseLevel;
  }
  if (typeof configuration.refreshAfterSave === 'undefined') {
    refreshAfterSave = defaultConfiguration.refreshAfterSave;
  }
  else {
    refreshAfterSave = configuration.refreshAfterSave;
  }

	createSettingsPage(autoCollapseLevel, refreshAfterSave);
	cleanUpLinks();
	fixQuoteLinksJQ();
	collapsibleZiggies();
...
function createSettingsPage(autoCollapseLevel, refreshAfterSave)
{
	...
	$('#loungePrefsToggle').text('Edit Lounge Preferences')
		...
		.click(
			...
			{
				e.preventDefault();
				$('#autoCollapseLevel').val(autoCollapseLevel);
				if(refreshAfterSave)
				{
				...
				// At this point it would be nice to scroll to
				// the bottom but I can't figure out how to do
				// it reliably.
			}
		);
	...
	$('#loungePrefsSave').click(
      ...
      var newConfiguration = {};
			var autoCollapseLevelFromBox = parseInt($('#autoCollapseLevel').val(),10);
			if(isFinite(autoCollapseLevelFromBox))
			{
				newConfiguration.autoCollapseLevel = autoCollapseLevelFromBox;
        autoCollapseLevel = autoCollapseLevelFromBox;
			}
			else
			{
				newConfiguration.autoCollapseLevel = autoCollapseLevel;
			}
			if($('#refreshAfterSave').attr('checked'))
			{
        newConfiguration.refreshAfterSave = true;
			}
			else
			{
				newConfiguration.refreshAfterSave = false;
			}
      refreshAfterSave = newConfiguration.refreshAfterSave;
      configurationJSON = JSON.stringify(newConfiguration);
      console.log("Saving new Lounge configuration: " + configurationJSON)
      GM.setValue("betterLoungeConfiguration", configurationJSON).then(reloadOrHide(refreshAfterSave), logRejection);
		}
	);
...
function logRejection(promiseValue) {
  console.log("Promise rejected: " + promiseValue);
}

function reloadOrHide(refreshAfterSave, promiseValue) {
  console.log("Promise fulfilled: " + promiseValue);
  if (refreshAfterSave) {
    location.reload()
  }
  else {
